...
class TestPersistDocsHudi(DBTSparkIntegrationTest):
    @property
    def schema(self):
        return "persist_docs_columns"
    ...
    def test_hudi_comments(self):
        self.run_dbt(['seed'])
        self.run_dbt(['run'])

        for table in ['table_hudi_model', 'seed']:
            results = self.run_sql(
                'describe extended {schema}.{table}'.format(schema=self.unique_schema(), table=table),
                fetch='all'
            )
    ...
    def test_hudi_comments_apache_spark(self):
        self.test_hudi_comments()
